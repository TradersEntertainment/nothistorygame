...
func snap_photo(who: String) -> String:
	...
	col.add_child(pic)
	var cap := Label.new()
	cap.text = (tr("UI_SIEGE_CAPTION") if who.begins_with("siege") else tr("UI_PHOTO_CAPTION")) % tr("PHOTO_" + who.to_upper())
